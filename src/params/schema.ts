...
/**
 * Sections of the Simple panel — plain language, no jargon.
 *
 * Deliberately independent of GROUPS: a control may sit in a different section
 * here than in its expert group, because the expert grouping follows the render
 * pipeline and the simple one follows what a person is trying to change.
 */
export const SIMPLE_SECTIONS = ['Pattern', 'Color', 'Light & Material', 'Finish'] as const;
...
interface Base {
  ...
  /**
   * Present = this control survives into the Simple panel, under this section
   * and this name. Absent = expert only. Simple never remaps a value — the
   * control writes the same parameter either way, so switching modes cannot
   * change the image.
   */
  readonly simple?: {
...
export const PARAM_SCHEMA = [
  ...
  { kind: 'slider', key: 'seed', label: 'Seed', group: 'Composition', uniform: 'uSeed',
    ...
    simple: { section: 'Pattern', label: 'Seed',
      hint: 'Every number gives a completely different pattern. The same number always gives the same one back.' } },
  { kind: 'slider', key: 'zoom', label: 'Zoom', group: 'Composition', uniform: 'uZoom',
    min: 0.15, max: 6, step: 0.01, default: 1, random: [0.5, 2],
    simple: { section: 'Pattern', label: 'Zoom', hint: 'Move closer to the pattern, or further away from it.' } },
  ...
  { kind: 'select', key: 'shape', label: 'Shape', group: 'Composition', uniform: 'uShape',
    ...
    simple: { section: 'Pattern', label: 'Shape',
      hint: 'Pours the liquid into a shape and flattens everything around it.' } },
  ...
  { kind: 'slider', key: 'scale', label: 'Scale', group: 'Flow', uniform: 'uScale',
    ...
    simple: { section: 'Pattern', label: 'Pattern Size',
      hint: 'A few large shapes, or many small ones.' } },
  { kind: 'slider', key: 'warp', label: 'Warp Strength', group: 'Flow', uniform: 'uWarp',
    ...
    simple: { section: 'Pattern', label: 'Swirl',
      hint: 'How much the pattern folds into itself. The single biggest change you can make here.' } },
  ...
  { kind: 'slider', key: 'octaves', label: 'Octaves', group: 'Flow', uniform: 'uOctaves',
    ...
    simple: { section: 'Pattern', label: 'Detail',
      hint: 'How much fine structure sits on top of the large shapes.' } },
  ...
  { kind: 'slider', key: 'motion', label: 'Motion', group: 'Flow', uniform: 'uMotion',
    ...
    simple: { section: 'Pattern', label: 'Motion',
      hint: 'How much the pattern moves while playing. The still image stays the same.' } },
  ...
  { kind: 'slider', key: 'relief', label: 'Relief', group: 'Surface', uniform: 'uRelief',
    ...
    simple: { section: 'Light & Material', label: 'Depth',
      hint: 'How raised the surface is — a gentle swell at the low end, deep folds at the high end.' } },
  ...
  { kind: 'select', key: 'material', label: 'Mode', group: 'Material', uniform: 'uMaterial',
    ...
    simple: { section: 'Light & Material', label: 'Material',
      hint: 'Metal reflects the light, glass lets it through.' } },
  ...
  { kind: 'slider', key: 'roughness', label: 'Roughness', group: 'Material', uniform: 'uRoughness',
    ...
    simple: { section: 'Light & Material', label: 'Polish',
      hint: 'Low is a mirror finish, high is soft and matte.' } },
  ...
  { kind: 'select', key: 'envMode', label: 'Environment', group: 'Lighting', uniform: 'uEnvMode',
    ...
    simple: { section: 'Light & Material', label: 'Lighting',
      hint: 'The room the surface is lit in. Each one gives a different mood.' } },
  { kind: 'slider', key: 'lightAngle', label: 'Light Angle', group: 'Lighting', uniform: 'uLightAngle',
    ...
    simple: { section: 'Light & Material', label: 'Light Direction',
      hint: 'Which side the main light comes from.' } },
  ...
  { kind: 'gradient', key: 'gradient', label: 'Gradient', group: 'Color', default: '0:000000,1:ffffff',
    ...
    simple: { section: 'Color', label: 'Colors',
      hint: 'The colours the image is built from. Presets… loads a ready-made set; click the bar to add your own.' } },
  ...
  { kind: 'slider', key: 'hueShift', label: 'Hue Shift', group: 'Color', uniform: 'uHueShift',
    min: -180, max: 180, step: 1, default: 0, random: [-180, 180],
    simple: { section: 'Color', label: 'Hue', hint: 'Rotates every colour around the colour wheel.' } },
  { kind: 'slider', key: 'saturation', label: 'Saturation', group: 'Color', uniform: 'uSaturation',
    ...
    simple: { section: 'Color', label: 'Saturation', hint: 'How strong the colours are. 0 is black and white.' } },
  ...
  { kind: 'slider', key: 'bloom', label: 'Bloom', group: 'Lens', uniform: 'uBloom',
    ...
    simple: { section: 'Finish', label: 'Glow',
      hint: 'Lets the bright areas bleed light into their surroundings.' } },
  ...
  { kind: 'slider', key: 'exposure', label: 'Exposure', group: 'Post', uniform: 'uExposure',
    ...
    simple: { section: 'Finish', label: 'Brightness', hint: 'Brightens or darkens the whole image.' } },
  { kind: 'slider', key: 'contrast', label: 'Contrast', group: 'Post', uniform: 'uContrast',
    min: 0.2, max: 3.5, step: 0.01, default: 1, random: [0.7, 2],
    simple: { section: 'Finish', label: 'Contrast', hint: 'How far apart the darks and the brights sit.' } },
  ...
  { kind: 'slider', key: 'vignette', label: 'Vignette', group: 'Post', uniform: 'uVignette',
    ...
    simple: { section: 'Finish', label: 'Vignette', hint: 'Darkens the corners of the frame.' } },
  ...
  { kind: 'slider', key: 'grain', label: 'Grain', group: 'Post', uniform: 'uGrain',
    ...
    simple: { section: 'Finish', label: 'Grain',
      hint: 'Film-like speckle. A little keeps large colour areas from banding.' } },
  { kind: 'color', key: 'background', label: 'Background', group: 'Post', uniform: 'uBackground',
    default: '#000000',
    simple: { section: 'Color', label: 'Background',
      hint: 'The colour behind the liquid. Visible wherever a Shape leaves room for it.' } },
...
/**
 * Drives the Simple panel. Order within a section follows schema order, which
 * reads sensibly for the current selection — add a numeric `simple.order` if
 * that ever stops being true, rather than reordering the schema itself.
 */
export const SIMPLE_BY_SECTION: ReadonlyArray<readonly [SimpleSection, readonly ParamDef[]]> =
  SIMPLE_SECTIONS.map(
    (s) => [s, PARAM_LIST.filter((d) => d.simple?.section === s && !d.hidden)] as const,
  );
